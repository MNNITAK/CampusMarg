...
const destinationTypes = [
  ...
  { id: 'laboratory', title: 'Laboratory', icon: <MaterialCommunityIcons name="flask" size={40} color="purple" />, description: 'Navigate to labs and research facilities' },
];
...
export default function DestinationSelect() {
  ...
  const handleSelect = (type: string) => {
    router.push(`/current-destination-details?type=${type}`);
  };

  return (
    <View style={styles.container}>
      <Text style={styles.title}>Look Around</Text>
       <Text style={styles.subtitle}>Tell us if it’s a cabin, a lab, or a class!</Text>
      ...
      <ScrollView style={styles.optionsContainer}>
        {destinationTypes.map((item, index) => (
          <TouchableOpacity
            ...
            style={[
              styles.optionCard,
              { transform: [{ translateY: index * 2 }] } // Subtle staggered effect
            ]}
